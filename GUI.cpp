...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), studentHashTable(25) {
    setupUI();
}
...
void MainWindow::setupUI() {
    ...
    yearInput->setPlaceholderText("Year of Study");
    formLayout->addWidget(yearInput);
    ...
    // criteriaComboBox->addItem("By Scores");
    // connect(criteriaComboBox, SIGNAL(currentIndexChanged(int)), this, SLOT(onCriteriaChanged(int)));
    histogramLayout->addWidget(criteriaComboBox);

    chartView = new QChartView(new QChart(), this);
    histogramLayout->addWidget(chartView);

    QPushButton *generateHistogramButton = new QPushButton("Generate Histogram", this);
